...
class GetSimilarMoviesUseCaseTest {
    ...
    @Test
    fun `test invoke should return list of similar movies`() = runBlocking {
        // Arrange
        val mockRepository = mockk<MovieRepository>() // Mock do MovieRepository
        val useCase = GetSimilarMoviesUseCase(mockRepository)
        ...
        coEvery { mockRepository.getSimilarMovies(movieId, apiKey) } returns expectedMovies
