...
class GetNowPlayingMoviesUseCaseTest {
    ...
    @Test
    fun `test invoke should return list of now playing movies`() = runBlocking {
        // Arrange
        val mockRepository = mockk<MovieRepository>() // Mock do MovieRepository
        val useCase = GetNowPlayingMoviesUseCase(mockRepository)
        ...
        coEvery { mockRepository.getNowPlayingMovies(apiKey) } returns expectedMovies
